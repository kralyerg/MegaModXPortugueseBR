...
1. Find the file with the text you want to fix. Everything is in the `UI/` and `Dialog/` folders, one file per building/toolbar/menu category. If you're not sure which file, use GitHub's search (press `/`) for a snippet of the English or Portuguese text.
...
3. Find the line you want to change. Each entry looks like this:
   ```
   { String _name = "SomeInternalName"; String _text = "Texto em português aqui"; }
   ```
...
```
á à â ã é ê í ó ô õ ú ç Á À Â Ã É Ê Í Ó Ô Õ Ú Ç º ª
```
No `ü`/`Ü` — modern Brazilian Portuguese orthography dropped the trema, so it's deliberately left out. Any other accented or special character (curly quotes, em dashes, letters from other languages, etc.) will not render and may break the build. Use plain straight quotes (`"`) and a regular hyphen (`-`) instead of "smart" typographic punctuation.
